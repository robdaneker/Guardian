...
public class NetworkRigidbody : MonoBehaviour {
	...
	public void setCharacter(ref float speed,ref float direction,ref float angle)
	{
		if(networkView.isMine)
		{
			Vector3 state = new Vector3(speed,direction,angle);
			networkView.RPC("networkCharacter", RPCMode.Others, state);	
		}
	}
	
	[RPC]
	public void networkCharacter(Vector3 state)
	{
		if(!networkView.isMine)
		{
			animator.SetFloat("Speed",state.x);
			animator.SetFloat("Direction",state.y);
			animator.SetFloat("Angle",state.z);
			if(state.x > 0.01f && (state.z>=135 || state.z<= -135)){
